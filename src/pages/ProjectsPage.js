...
export default function ProjectsPage({ onSelect, currentUser }) {
  ...
  return (
    <div style={{ maxWidth: 680, margin: "0 auto" }}>
      ...
      {loading
        ...
        : projects.length === 0
          ? <p style={{ fontSize: 14, color: "var(--text-tertiary)", textAlign: "center", padding: "2rem 0" }}>
              {isLeadOrAdmin ? "No hay proyectos todavía." : "No tenés proyectos asignados todavía."}
            </p>
          : projects.map(p => {
            const memberCount = p.project_members?.length || 0;
            return (
              <div key={p.id} style={{ background: "var(--card-bg)", border: "0.5px solid var(--border)", borderRadius: 12, padding: "1rem 1.25rem", marginBottom: 10 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 12 }}>
                  <div style={{ flex: 1, cursor: "pointer" }} onClick={() => onSelect(p)}>
                    ...
                    <div style={{ fontSize: 13, color: "var(--text-secondary)" }}>
                      {p.client} · {p.area} · {p.created_by}
                    </div>
                  </div>
                  <div style={{ display: "flex", gap: 6, flexShrink: 0 }}>
                    <button
                      onClick={() => setMembersProjectId(membersProjectId === p.id ? null : p.id)}
                      style={{ fontSize: 12, padding: "5px 10px", borderRadius: 8, border: "0.5px solid var(--border)", background: membersProjectId === p.id ? "#E6F1FB" : "transparent", color: membersProjectId === p.id ? "#0C447C" : "var(--text-secondary)", cursor: "pointer" }}
                    >
                      👥 {memberCount}
                    </button>
                    {isLeadOrAdmin && (
